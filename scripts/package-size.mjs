...
const reference = process.argv.includes('--write')
  ? report
  : JSON.parse(await readFile(reportURL, 'utf8'));
const sizeBlock = [
  '<!-- size:start -->',
  `**Zero dependencies. ${(reference.bundles.all.gzipBytes / 1000).toFixed(1)} kB min+gzip, including all providers.**`,
  '',
...
if (process.argv.includes('--write')) {
  ...
  const saved = JSON.parse(await readFile(reportURL, 'utf8'));
  assert.equal(saved.version, report.version, 'Size report version is stale.');
  assert.equal(saved.esbuild, report.esbuild, 'Size report bundler is stale.');
  for (const [name, size] of Object.entries(bundles)) {
    assert.equal(
      saved.bundles[name].sha256,
      size.sha256,
      `${name} bundle changed; run npm run size:write.`,
    );
    assert.equal(saved.bundles[name].minifiedBytes, size.minifiedBytes);
    if (saved.zlib === report.zlib) assert.equal(saved.bundles[name].gzipBytes, size.gzipBytes);
  }
}
if (!process.argv.includes('--write')) {
  for (const file of ['README.md', 'packages/sysone/README.md']) {
...
console.table(bundles, ['minifiedBytes', 'gzipBytes']);
console.log(
