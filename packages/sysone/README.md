...
```ts
import { predicate, classifier, rubric } from 'sysone-help';
...
Other open evaluation models include [Bespoke Nimble 9B](https://huggingface.co/bespokelabs/Bespoke-Nimble-9B) and [Kotoba Open-Jev DeBERTa](https://huggingface.co/com-kotobalabs/open-jev-deberta-v3-large). Both publish evaluation weights. Real inference was tested with Kotoba through a local Python/System One bridge and with Nimble through a Gradio demo bridge. These are experimental integrations outside the TS package. Nimble also has a native System One server, but its public endpoint returned 503 during testing. [Results and runnable checks](https://github.com/sysone-help/sysone/tree/main/research/validation). No official open Jev weights were found in our September 19, 2026 review. [Research and source links](https://github.com/sysone-help/sysone/blob/main/research/open-evaluation-models.md).
...
```ts
import type { EvaluationProvider } from 'sysone-help';
...
```ts
// 0.3
import { systemOne } from 'sysone-help/providers/system-one';
