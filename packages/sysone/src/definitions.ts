...
function text(value: string): void {
  if (typeof value !== 'string' || !value.trim()) throw new SysoneError('Instructions and criteria must be non-empty strings.', 'INVALID_INPUT');
}
...
export function predicate(instructions: string, criteria?: Predicate['criteria']): Predicate {
  text(instructions);
  if (criteria) { text(criteria.true); text(criteria.false); }
  return Object.freeze({ type: 'boolean', instructions, ...(criteria ? { criteria: Object.freeze({ ...criteria }) } : {}) });
}
...
export function classifier<const Choices extends Record<string, string>>(
  ...
  if (entries.length < 2 || entries.length > 255) throw new SysoneError('A classifier needs 2–255 choices.', 'INVALID_INPUT');
  for (const [key, value] of entries) { text(key); text(value); }
...
export function rubric(instructions: string, levels: readonly string[]): Rubric {
  text(instructions);
  if (levels.length < 2 || levels.length > 10) throw new SysoneError('A rubric needs 2–10 ordered levels.', 'INVALID_INPUT');
  levels.forEach(text);
